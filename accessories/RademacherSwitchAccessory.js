...
RademacherSwitchAccessory.prototype.getCurrentState = function(callback) {
    if (this.debug) this.log("%s [%s] - getting current state", this.accessory.displayName, this.sw.did);
...
RademacherSwitchAccessory.prototype.setCurrentState = function(value, callback) {
    if (this.debug) this.log("%s [%s] - setting switch: %s", this.accessory.displayName, this.sw.did,value);

    var self = this;
    this.currentState = value;
    var changed = (this.currentState != this.lastState);
    if (this.debug) this.log("%s [%s] - switch changed=%s, lastState=%s", this.accessory.displayName, this.sw.did,changed, this.lastState);
    if (changed)
    {
